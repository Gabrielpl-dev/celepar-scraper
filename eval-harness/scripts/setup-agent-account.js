#!/usr/bin/env node
// Cria (uma única vez) a conta de serviço usada pelo eval-harness -- espelha
// mcp-server/scripts/setup-agent-account.js de propósito, mesma lógica, conta separada.
...
const AGENT_USERNAME = process.env.CELEPAR_EVAL_USERNAME || 'AGENTE_EVAL'
...
async function main() {
  ...
  const agentPassword = process.env.CELEPAR_EVAL_PASSWORD || await ask(`Senha pra nova conta "${AGENT_USERNAME}": `)
  ...
  console.log(`Preencha o eval-harness/.env com:\n  CELEPAR_EVAL_USERNAME=${AGENT_USERNAME}\n  CELEPAR_EVAL_PASSWORD=<a senha que você acabou de digitar>\n`)
}
